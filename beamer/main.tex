...
layers.  
图三是层合材料的结构图
对于层合材料来说，它的强度是由材料的铺层角，材料的性质和材料的厚度决定。
...
在生产中，使用的层合材料除了需要满足工程强度的要求以外，还需要对生产成本和材料本身的
质量要求。
...
\note{
	...
	和约束条件的关系调整演化的方向和速度。
	在这个公式里，
	The second technique in the proposed genetic algorithm is self-adaptive mutation
...
\end{frame}
\note{
	在这里展现的是我们一部分的实验成果，从这里可以看出使用改进的遗传算法可以获得
	实验结果是更好的。
}


\note{
	We have three conclusions from this chapter:
...
\note{
	那么需要什么样结构的网络来预测复合材料的强度呢？我们推荐了一下的拓扑结构。该网络有以下
	几个特点
...
\note{
 ...
 error两个指标上都比全连接神经网络有更好的数据表现。并把我们的研究成果发表在计算机类
 的期刊上。。 
